...
6. **感情状態**: 会話全体のトーンと内容に基づいて、エージェントの感情状態を更新してください。次の JSON ファイルの `emotional_state` フィールドには、必ず以下 8 値のいずれかを使用してください: `平常`、`好奇心`、`集中`、`達成感`、`高揚`、`思索`、`警戒`、`困惑`。英語の単語（`focused`、`excited` など）や、リストに無い表現は使用しないでください。
  {profile_path}
